...
# Function to update kubeconfig for EKS clusters with profile support
eks-config() {
  if [ -z "$1" ]; then
    echo "Please provide a cluster name"
    echo "Usage: eks-config <cluster-name> [profile] [region]"
    return 1
  fi

  # First parameter is always the cluster name
  local cluster=$1
  # Second parameter is profile (if provided)
  local profile=${2:-default}
  # Last parameter is region (defaults to ap-southeast-1)
  local region=${3:-ap-southeast-1}

  echo "Updating kubeconfig for cluster: $cluster using profile: $profile in region: $region"
  aws eks --region $region --profile $profile update-kubeconfig --name $cluster
}
